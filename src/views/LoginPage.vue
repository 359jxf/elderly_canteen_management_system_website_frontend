    ...
    async login() {
      ...
      try {
        ...
        if (res.data.success) {
          ...
          // 设置用户头像，如果用户信息中有头像则使用用户头像，否则使用默认头像
          const userAvatar = res.data.response.avatar || new URL('../assets/defaultportrait.png', import.meta.url).href;

          // 将 Token 和用户信息存储在 localStorage
