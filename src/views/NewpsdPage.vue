  ...
  <div class="container">
    <div class="wrapper">
      ...
      <div class="inputpart" v-if="step === 1">
        ...
        <input type="text" v-model="localPhone" id="phone" placeholder="手机号" :disabled="isPhonePreFilled"/>
...
export default {
  ...
  data() {
    return {
      ...
      isPhonePreFilled: false, // 标识手机号是否已预填
    };
  },
  created() {
    console.log('Received phone:', this.phone); // 调试输出
    if (this.phone) {
      this.isPhonePreFilled = true;
      this.localPhone = this.phone; // 设置本地 phone 的值
      this.sendOTP(); // 自动发送验证码
    } 
  },
  methods: {
    sendOTP() {
    ...
    resetPassword() {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          } else if (this.from === 'change-password') {
            this.$router.push("/profile"); // 假设个人信息页路径为 /profile
          }
        }, 2000);
      }, 1000);
    },
